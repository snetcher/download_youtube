...
def download_youtube_video(url: str, output_path: str = DEFAULT_DOWNLOAD_PATH) -> None:
    ...
    video_file = os.path.join(tmp_path, f"{base_name}_video.mp4")
    audio_file = os.path.join(tmp_path, f"{base_name}_audio.mp4")
    ...
    try:
        ...
        video_stream.download(output_path=tmp_path, filename=f"{base_name}_video.mp4")
        print("Video downloaded.")

        print("Downloading audio stream...")
        audio_stream.download(output_path=tmp_path, filename=f"{base_name}_audio.mp4")
